...
\author{\textcolor{umBlueLighter}{Tatiana Boura}} % Your name
\institute[NCSR Demokritos \& University of Piraeus]
{\noindent
    Supervisor: \textcolor{umBlueLighter}{Nikos Katzouris}\par
    \vspace{1em}
...
% Range is from 6 to 10 for a reason
    \begin{itemize}
        \setlength{\itemsep}{16pt}
        \item To generate the positive instances, from the overtake instances we got chunks of frames of \textbf{minimum length 6} and\textbf{ maximum 10} %with 5 frames in between the chunks (less for smaller instances)
...
\begin{frame}{Train and Test splits}
    \begin{itemize}
        ...
        \end{itemize}
        \item We focused on testing with \colorbf{out-of-distribution} data
...
\begin{frame}{Automaton 2 - Complex}
    \begin{columns}
        ...
        \column{0.52\textwidth}
         \includegraphics[width=\textwidth]{contents/images/autom_2 - Copy.png}
    \end{columns}
\end{frame}
...
\begin{frame}{Motivation for the rest of the Background}
    \begin{itemize}
        \setlength{\itemsep}{12pt}
        \item Our goal is to perform CER on \textbf{sub-symbolic} data%(distributed, numerical representations less transparent to human understanding)
        \item So, we use NNs for the sub-symbolic \textit{simple events} (action, location) and allow \textit{complex event} recognition (overtake) to be handled by the automaton
        ...
        \vspace{0.6em}
            ...
            \setlength{\itemsep}{4pt}
                ...
                \item \colorbf{Why?}  Differentiable circuits answer probabilistic queries and train the NN to maximize the log-likelihood of the correct prediction for these queries.
...
\begin{frame}{Finite Automata}
    \begin{columns}
        % Left column: Finite Automaton definition
        \column{0.6\textwidth}
        A (deterministic) \textbf{finite automaton} (FA) is a 5-tuple $(\mathbb{Q}, \Sigma, \delta, q_0, \mathbb{F})$, where
        \vspace{0.6em}
        \begin{enumerate}
            \item $\mathbb{Q}$ is a finite set called the \textcolor{umBlueLighter}{states},
            \item $\Sigma$ is a finite set called the \textcolor{umBlueLighter}{alphabet},
...
\begin{frame}{Non-stationary Markov Chains}
    \begin{itemize}
        \setlength{\itemsep}{12pt}
         ...
         \item \colorbf{Key Concept}: A limited number of previous states is needed to predict the future ones
...
\begin{frame}{Loss Functions}
    ...
    \item We switched to \colorbf{focal loss}, which down-weights easy examples and emphasizes difficult ones
     $$\text{FL}(p_t) = - \alpha_t \cdot (1 - p_t)^\gamma \cdot \log(p_t),$$
        where $p_t=p$ if the true label is 1, and $p_t=1-p$ if the true label is 0
    \end{itemize}
\end{frame}

\begin{frame}{Loss Functions}% Adjust width as needed}
    %\vspace{-2em}
    ...
    \begin{itemize}
        ...
        get \colorbf{confident predictions}
    \end{itemize}
...
\begin{frame}{Results - Baseline vs NeSy}
    \begin{itemize}
    \setlength{\itemsep}{10pt}
    \item In all experiments the main evaluation metric is \textbf{F1-micro}
    \item \colorbf{[Cross-Validation]} :
    \end{itemize}
\begin{table}[H]
	\centering
	\scalebox{1.0}{
...
\begin{table}[H]
	...
	\begin{tabular}{cccccc}
		...
		2 & 12 & 3.25 & 7/14 & 0.542 & 0.391 \\
		3 & 14 & 3.85 & 11/12 & 0.523 & 0.574\\
		\bottomrule
	\end{tabular}
	}
\end{table}
...
\begin{frame}{Results - Training Times / Epoch}
   \begin{table}[H]
	...
	\begin{tabular}{ccccccc}
		\toprule
		& \multicolumn{4}{c}{\cellcolor{umYellow!80}\textbf{Statistics}} & \cellcolor{umOrange}\textbf{Baseline} & \cellcolor{umBlueLighter}\textbf{NeSy} \\
		\multirow{-3}{*}{Automaton} & \cellcolor{umYellow!80}\#trans & \cellcolor{umYellow!80}avg. lit/trans & \cellcolor{umYellow!80}\#unique simple ev. & \cellcolor{umYellow!80} SDD size & \cellcolor{umOrange} tt/e & \cellcolor{umBlueLighter} tt/e \\
		\midrule
		1 & 10 & 1.2 & 4/14 & 427 & 0.15 mins  &\textbf{1.5 mins} \\
		2 & 12 & 3.25 & 7/14 & 565 & 0.15 mins   &\textbf{6 mins} \\
		3 & 14 & 3.85 & 11/12 & 478 & 0.15 mins  &\textbf{4.5 mins}\\
		\bottomrule
	\end{tabular}
	}
\end{table}
   \vspace{1em}
\begin{itemize}
\item NeSy's \textbf{time breakdown}: 40\% probabilistic inference, 40\% backpropagation, 20\% rest
\end{itemize}
\end{frame}
...
\begin{table}[H]
	...
	\scalebox{1.0}{
		\begin{tabular}{l|ccc}
			\toprule
             & \multicolumn{2}{c}{\cellcolor{umBlueLighter!70!white} \textbf{True Label}} & \cellcolor{umYellow!70} \textbf{Pre-trained}\\
			 & \cellcolor{umBlueLighter!70!white} 1-hot encoding & \cellcolor{umBlueLighter!70!white} Large Prob. Mass  (0.8) & \cellcolor{umYellow!70} Prob. Distribution (macro-avg)\\
			\midrule
			 \cellcolor[HTML]{FFFDF6}\textbf{F1-micro} & \textbf{0.947} &  \textbf{0.561} & \textbf{0.026}\\
			\bottomrule
		\end{tabular}
	}
\end{table}
...
\begin{table}[H]
	...
	\scalebox{0.8}{
		\begin{tabular}{cccc}
			\toprule
			& \cellcolor{umOrange}\textbf{Neural Baseline} & \cellcolor{umBlueLighter}\textbf{NeSy} & \cellcolor{umYellow}\textbf{Pre-trained + Automaton}\\
			\multirow{-2}{*}{\begin{tabular}[c]{@{}l@{}}Data split \\ index\end{tabular}} & \cellcolor{umOrange} F1-micro & \cellcolor{umBlueLighter}F1-micro & \cellcolor{umYellow}F1-micro\\
			\midrule
			\multicolumn{3}{c}{\textit{\textbf{2D}}} \\
			\midrule
			1 & 0.485 & \textbf{0.632} & 0.0\\
			2 & 0.3 & \textbf{0.649} & 0.0\\
			3 & 0.446 & \textbf{0.649} & 0.0\\
			4 & 0.457 & \textbf{0.612} & 0.0\\
			\midrule
			\multicolumn{3}{c}{\textit{\textbf{3D}}} \\
			\midrule
			1 & 0.549 & \textbf{0.657} & 0.214\\
			2 & \textbf{0.541} & 0.342 & 0.0\\
			3 & 0.474 &  \textbf{0.666} & 0.0\\
			4 & 0.454 & \textbf{0.588} & 0.0\\
			\bottomrule
		\end{tabular}
	}
\end{table}
...
\vspace{1em}
\colorbf{When trained on simple events}:
...
\begin{frame}{Future Work}
    \begin{itemize}
        ...
        \item Try other simple event recognition \textbf{architectures}, \textit{e.g., transformers}
